...
import { formatDate, formatRelativeDate, formatSize, randomString } from "tutils/misc"
import { containsPath, getDir, getExt, getName } from "tutils/path"
import { open } from "tutils/process"
import { normalizeURL, resolveURL } from "tutils/url"
import { runInVM } from "tutils/vm"
import { parse } from "url"
import { getDefaultIcon, getDirIcon, getFileIcon } from "./icon"
...
export class WebServer extends HTTPServer {
	...
	constructor(options: WebServerOptions = {}) {
		super(formatHTTPServerOptions(options))
			...
			if (!process) {
				...
				if (staticPath) {
					if (typeof staticPath === "string") {
						process = (request, response) => this.writeStatic(request, response, this.formatURL(staticPath, request.path, matcher.base) + (request.search ? "?" + request.search : ""))
					} else {
						process = (request, response, server) => this.writeStatic(request, response, staticPath(request, response, server))
					}
				} else {
					...
					if (rewrite) {
						...
						if (proxy) {
							if (typeof proxy === "string") {
								process = (request, response) => this.writeProxy(request, response, this.formatURL(proxy, request.path, matcher.base) + (request.search ? "?" + request.search : ""))
							} else {
								process = (request, response, server) => this.writeProxy(request, response, proxy(request, response, server))
							}
						} else {
							process = () => { }
						}
		...
		function formatHTTPServerOptions(options: WebServerOptions) {
	...
	start() {
		return new Promise<void>((resolve, reject) => {
			...
			this.listen(this.port, this.hostname, this.backlog, () => {
				...
				if (this.open) {
					open(resolveURL(this.url!, this.openURL), false, typeof this.open === "string" ? this.open : undefined)
				}
				resolve()
			})
		})
		...
		try {
			...
			for (const router of this.routers) {
				if (router.matcher.test(request.path, request, response, this)) {
					...
					if (router.break !== undefined ? router.break : response.finished) {
						break
					}
				}
			}
			// 如果没有路由发送 HTTP 响应头，则按静态文件处理
	...
	async writeStatic(request: HTTPRequest, response: HTTPResponse, path: string) {
		let stat: Stats
		try {
			stat = await this.fs.getStat(path)
		} catch (e) {
			if (e.code !== "ENOENT") {
				this.writeError(request, response, e)
			}
		...
		if (stat.isFile()) {
			await this.writeFile(request, response, path, `${stat.size.toString(36)}${stat.mtimeMs.toString(36)}`)
		} else if (stat.isDirectory()) {
			if (!request.path.endsWith("/")) {
				response.setHeader("Location", `${encodeURI(request.path)}/${request.search ? "?" + request.search : ""}`)
				response.writeHead(301, this.headers)
				response.end()
				return
			}
			const entries = await this.fs.readDir(path)
			for (const index of this.defaultPages) {
				if (this.fs.isCaseInsensitive ? entries.some(entry => entry.toLowerCase() === index.toLowerCase()) : entries.includes(index)) {
					request.path += index
			...
			if (this.directoryList) {
				await this.writeDir(request, response, (await Promise.all(entries.map(async entry => {
					try {
	 ...
	 * @param etag 文件最后修改戳
	 */
	writeFile(request: HTTPRequest, response: HTTPResponse, path: string, etag?: string) {
		if (etag) {
...
<body>
	...
	<ul>
		...
		${entries.map(entry => `<li><a title="${encodeHTML(entry.title || entry.name)}" href="${encodeHTML(entry.name)}${entry.isDir ? "/" : ""}"><span class="size">${entry.size ? formatSize(entry.size) : "&nbsp;"}</span><span class="time" title="${entry.modified ? entry.modified.toLocaleString() : ""}">${entry.modified ? this.formatDate(entry.modified, now) : ""}</span>${entry.isDir ? getDirIcon(entry.name) : getFileIcon(entry.name)}${encodeHTML(entry.name)}${entry.isDir ? "/" : ""}</a></li>`).join("\n")}
	</ul>
</body>
</html>`
	 ...
	 */
	writeContent(request: HTTPRequest, response: HTTPResponse, content: string | Buffer, mimeType = "application/octet-stream", etag?: string) {
		if (etag) {
			const ifNoneMatch = request.headers["if-none-match"]
			if (ifNoneMatch === etag) {
				response.writeHead(304, this.headers)
	 ...
	 * @param end 是否结束请求
	 */
	async writeServerJS(request: HTTPRequest, response: HTTPResponse, path: string, context?: { [key: string]: any }, end = true) {
	...
	async writeEJS(request: HTTPRequest, response: HTTPResponse, path: string, context?: { [key: string]: any }, end = true) {
		...
		code.replace(/<%(#?)(=?)(.*?)%>/sg, (source, comment: string | undefined, equal: string | undefined, text: string, index: number) => {
			...
			compiled += equal ? `; __output__ = ${text}; if (__output__ != null) { response.write(Buffer.isBuffer(__output__) ? __output__ : String(__output__)); }` : text
